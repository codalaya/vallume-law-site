...
function unProtectedGeneralSettings()
{
    $settings = \App\Models\GeneralSetting::where('is_protected',0)->select('name', 'value','is_protected')->pluck('value', 'name')->toArray();
    return $settings;
}
...
function hexToRgb($hexColor) {
    ...
    // Ensure that the hex color is valid (6 or 3 characters)
    if (!preg_match('/^[a-f0-9]{3}$|^[a-f0-9]{6}$/i', $hexColor)) {
        return null; // Return null or handle error as needed
    }

    // Convert short hex to full form (e.g., #ABC -> #AABBCC)
