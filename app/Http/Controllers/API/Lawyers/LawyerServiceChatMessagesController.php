...
class LawyerServiceChatMessagesController extends Controller
{
    ...
    public function getChatMessages(Request $request , BookedService $booked_service)
    {
       $user = Auth::user();
       $lawyer = $user->lawyer;
      if($booked_service->lawyer_id != $lawyer->id){
    ...
    public function sendChatMessage(SendMessageRequest $request)
    {
         ...
         $data = $request->all();
         $data['sender_id'] = $lawyer->id;
         $data['sender_type'] = 'App\Models\Lawyer';

         $data['reciever_id'] = $booked_service->customer_id;
