...
class WalletController extends Controller
{
    ...
    public function AddAmountToWallet(Request $request)
    {
        ...
        if ($request->gateway == 'stripe') {
            ...
            // dd($data['fund_id']);
            return Inertia::location(route('customers.appointment_stripe_transfers_for_wallet', ['fund_id' => $data['fund_id']]));
        }
    ...
    public function payThroughUserWallet($amount,$request = null)
    {
        $user = Auth::user();
        if ($user->balanceInt > 0 && $user->balanceInt > $amount || $user->balanceInt == $amount) {
            if($request  && $request->type == 'service'){
                $meta = ['details' => 'Service Bought through Wallet','service_id' => $request->service_id];
            }
            else{
