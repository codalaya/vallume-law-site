...
use App\Models\Post;
use App\Http\Requests\Lawyers\CommunityPosts\CreateRequest;
...
class CommunityPostsController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(['auth','lawyer'],['only' => ['store']]);
        $this->middleware(['auth','lawyer'],['only' => ['postLike']]);
        $this->middleware(['auth','lawyer'],['only' => ['postComment']]);
        $this->middleware(['auth','lawyer'],['only' => ['commentReply']]);
        $this->middleware(['auth','lawyer'],['only' => ['import']]);
    ...
    public function getter($req = null, $export = null)
    {
        ...
        if ($req != null) {
            ...
            $totalCommunityPosts = $lawyer_posts->count();
            $lawyer_posts = $lawyer_posts->paginate($req->perPage);
            $lawyer_posts = CommunityPostsResource::collection($lawyer_posts)->response()->getData(true);

            return $lawyer_posts;
        }
        $lawyer_posts = CommunityPostsResource::collection($lawyer->lawyer_posts()->withAll()->orderBy('id', 'desc')->paginate(10))->response()->getData(true);
        return $lawyer_posts;
    }
    ...
    public function filter(Request $request)
    {
        ...
        $response = generateResponse($lawyer_posts, count($lawyer_posts['data']) > 0 ? true : false, 'Filter CommunityPosts Successfully', null, 'collection');
        return response()->json($response, 200);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->is_shared){
            try {
                DB::beginTransaction();
                $request->merge(['created_by_user_id' => auth()->user()->id,'description' => $request->sharedtext,'share_with' => $lawyer->id]);
                $data = $request->all();
                if($request->sharedImg){
                    $data['image'] = $request->sharedImg;
                }
                $lawyer_post = $lawyer->lawyer_posts()->create($data);
                $lawyer_post->slug = Str::slug(Str::limit($lawyer_post->description, 20) . '-' . $lawyer_post->id);
                $lawyer_post->save();
                $lawyer_post = $lawyer->lawyer_posts()->withAll()->find($lawyer_post->id);
                $lawyer_post = new CommunityPostsResource($lawyer_post);
    ...
    {
        $lawyer_id = $post->lawyer_id ?? auth()->user()->id;

        $existing_like = $post->post_likes()->where('lawyer_id', $lawyer_id)->first();
    ...
    public function postComment(Request $request, Post $post)
    {
        ...
        $lawyer = auth()->user()->lawyer;
        $request->merge(['user_id' => $auth_id, 'content' => $request->comment,'lawyer_id' => $lawyer?->id]);
