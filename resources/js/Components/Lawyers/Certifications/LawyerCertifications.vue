<template>
  ...
  >
    <Table v-if="!this.fetching">
      ...
      <template #TableBody>
        <tr v-if="lawyer_certifications.data.length == 0">
            <td class="align-middle" :colspan="columns.length">
                {{ __('no record found') }}
            </td>
        </tr>
        <tr v-for="certification in lawyer_certifications.data" :key="certification.id">
          <td class="align-middle">{{ certification.name }}</td>
          <!-- <td class="align-middle" v-html="certification.description"></td> -->
          <td class="align-middle">
              <a class="text-primary" v-if="certification.image" target="_blank" :href="certification.image">{{ __('download') }}</a>
<!--            <img v-if="certification.image" :src="certification.image" width="75" height="75" :alt="certification.image" />-->
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_certifications.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditCertificationModal from "@/Components/Lawyers/Certifications/AddEditCertificationModal.vue";
import ViewCertificationModal from "@/Components/Lawyers/Certifications/ViewCertificationModal.vue";
import DeleteCertificationModal from "@/Components/Lawyers/Certifications/DeleteCertificationModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false){
        await this.getLawyerCertifications(loading_more)
     },
     clearModalData(){
        this.modal_certification = null
        this.key++
     },
    getLawyerCertifications(loading_more){
    axios.post(this.route('lawyers.lawyer_certifications.filter'),this.filter).then(res => {
        const data = res.data.data
        if(loading_more){
            this.lawyer_certifications.data = this.lawyer_certifications.data.concat(data.data);
        }else{
            this.lawyer_certifications.data = data.data;
        }
        this.lawyer_certifications.links = data.links
        this.lawyer_certifications.meta = data.meta
        this.fetching = false
    });
    },
  }
