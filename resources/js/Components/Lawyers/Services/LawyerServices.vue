<template>
  <div class="tab-pane" :class="{ active: active }" id="services" role="tabpanel" aria-labelledby="services-tab"
    ...
    <Table v-if="!this.fetching">
      ...
      <template #TableHeaderButtons>
        <div class="d-flex align-items-center">
          ...
          <div class="from-group me-2 position-relative">
            <input v-model="filter.search" class=" form-control  px-3" placeholder="Search" type="text" />
      ...
      <template #TableBody>
        <tr v-if="lawyer_services.data.length == 0">
          <td class="align-middle" :colspan="columns.length">
            {{ __('no record found') }}
          </td>
        </tr>
        <tr v-for="service in lawyer_services.data" :key="service.id">
          ...
          <td class="align-middle">
            <div class="d-flex">
              <button type="button" class="btn btn-link px-1 lh-1 py-1 me-2" data-bs-toggle="modal"
              ...
              <button type="button" class="btn btn-link text-danger px-1 lh-1 py-1 " data-bs-toggle="modal"
      ...
      <template #Pagination>
        <TablePagination @onPageChange="onPageChange" :meta="lawyer_services.meta"></TablePagination>
      </template>
    </Table>
  </div>
</template>
...
import AddEditServiceModal from "@/Components/Lawyers/Services/AddEditServiceModal.vue";
import ViewServiceModal from "@/Components/Lawyers/Services/ViewServiceModal.vue";
import DeleteServiceModal from "@/Components/Lawyers/Services/DeleteServiceModal.vue";
...
export default defineComponent({
  ...
  methods: {
    async getPaginatedData(loading_more = false) {
      await this.getLawyerServices(loading_more)
    },
    clearModalData() {
      this.modal_service = null
      this.key++
    },
    getLawyerServices(loading_more) {
      axios.post(this.route('lawyers.lawyer_services.filter'), this.filter).then(res => {
        const data = res.data.data
        if (loading_more) {
          this.lawyer_services.data = this.lawyer_services.data.concat(data.data);
        } else {
          this.lawyer_services.data = data.data;
        }
        this.lawyer_services.links = data.links
        this.lawyer_services.meta = data.meta
        this.fetching = false
      });
    },
  }
